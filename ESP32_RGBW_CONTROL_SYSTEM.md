...
系统支持两种通信协议：JSON协议和二进制协议。在连接建立后，客户端会自动协商使用最优协议。

#### 协议协商

客户端发送：
```json
{"cmd": "protocol_negotiate"}
```

服务器响应：
```json
{"cmd": "protocol_supported", "json": true, "binary": true}
```
...
| `ping` | 心跳包 | 无 |
| `pong` | 心跳响应 | 无 |
| `protocol_negotiate` | 协议协商 | 无 |
| `protocol_supported` | 协议支持响应 | `json`, `binary` |
...
为提高性能，特别是在高频更新场景下，系统支持二进制协议。

#### 二进制协议结构

```
| 命令字 (1字节) | 数据长度 (1字节) | 参数1 (n字节) | 参数2 (n字节) | ... |
```
...
| 命令字 | 命令名称 | 数据长度 | 参数说明 |
|--------|----------|----------|----------|
| 0x01 | 静态颜色 | 5 | r, g, b, w, brightness |
| 0x02 | 效果控制 | 7 | effect_type, speed, r, g, b, w, brightness |
| 0x03 | 开关控制 | 1 | on_state (0=关, 1=开) |
| 0x04 | 心跳包 | 0 | 无 |
| 0x05 | 获取状态 | 0 | 无 |
...
1. **设置静态颜色** (红色，亮度80%)
   ```
   [0x01, 0x05, 0xFF, 0x00, 0x00, 0x00, 0x50]
   ```

2. **设置呼吸效果** (蓝色，速度50，亮度70%)
   ```
   [0x02, 0x07, 0x01, 0x32, 0x00, 0x00, 0xFF, 0x00, 0x46]
   ```
...
| 协议类型 | 消息大小 | 解析速度 | 适用场景 |
|----------|----------|----------|----------|
| JSON | 较大 (~100-200字节) | 较慢 | 一般控制场景 |
| 二进制 | 较小 (~5-10字节) | 极快 | 高频更新场景（如音乐律动） |
